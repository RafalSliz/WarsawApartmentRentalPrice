...
function onClickedEstimatePrice() {
    ...
    if (sqft.value < 20) {
        window.alert("Proszę wybrać powierzchnię nie mniejszą niż 20");
        return false;
    };
    if (sqft.value > 150) {
        window.alert("Proszę wybrać powierzchnię nie większą niż 150");
        return false;
    };
    if (sqft.value >= 20 && sqft.value <= 150) {
        $.post(url, {
...
function onPageLoad() {
    ...
    //var url = "http://127.0.0.1:5000/get_location_names"; // local server
